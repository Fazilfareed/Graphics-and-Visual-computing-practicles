...
//compute the outcode of a given point 1010
...
void cohenSutherland(int x0, int y0, int x1, int y1) {
    ...
    outcode0 = computeOutCode(x0, y0); //computing 0010 A
    outcode1 = computeOutCode(x1, y1); //computing 0001 B

    do {
        if (!(outcode0 | outcode1)) { // Trivially accepted //if or operator 0000 A|B==!false
            accept = true;
            done = true;
        } else if (outcode0 & outcode1) { // Trivially rejected //if and operator !=0000 
            done = true;
        } else {
            float x, y;
            outcode = outcode0 ? outcode0 : outcode1; //if outcode0 is in middle take outcode1
			...
			//checking what we took and change the values for second line
            if (outcode == outcode0) {
                x0 = x;
                y0 = y;
